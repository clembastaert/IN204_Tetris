...
void Game::HomeScreen() {
  ...
  while (window.pollEvent(event)) {
    ...
    if (event.type == sf::Event::TextEntered) {
      ...
      else if (event.text.unicode > 47 && event.text.unicode < 126 &&
...
void Game::MultiPlayerScreen() {
  ...
  while (window.pollEvent(event)) {
    ...
    if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
      ...
      // Button to select the password field
    ...
    if (!current_block->go_down(grid)) {
      ...
      if (points > 0) {
        ...
        if (player.get_score() >= player.get_level() * 1000 &&
          ...
          fps_grid = fps_grid_init * sqrt(float(new_level)) * 1.f;
        }
      }
      ...
      if (is_end_game()) {
        end_game = !end_game;
        if (player.isClient()) {
          for (size_t n = 0; n < pseudos_others_players.size(); n = n + 1) {
            setTextCenterPosition(
                *pseudos_others_players[n],
                {WIN_WIDTH * 0.5f, WIN_HEIGHT * 0.4f + n * 0.08f * WIN_HEIGHT});
          }
          screen = 3;
        }
...
void Game::Initialize_graphics() {
  ...
  sf::Color title_cases_color = sf::Color(243, 190, 255);
  ...
  initialize_text(end_msg_, main_font_, 1, WindowView.getCenter(),
                  0.2 * (WIN_WIDTH - 4 * e - 2 * L_cases), "PARTIE FINIE",
                  sf::Color::White, {1, 0.7});
  initialize_text(try_again_, main_font_, 1,
                  {WindowView.getCenter().x, WindowView.getCenter().y * 5 / 4},
                  0.1 * (WIN_WIDTH - 4 * e - 2 * L_cases),
                  "Cliquer pour recommencer", sf::Color::White, {1, 0.7});
